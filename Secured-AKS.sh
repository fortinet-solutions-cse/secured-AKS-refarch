...
az network vnet create  --name ftnt-demo-aks  --resource-group $GROUP_NAME  \
  --subnet-name ftnt-demo-aks-sub --address-prefix 10.40.0.0/16 --subnet-prefix 10.40.0.0/16

az network route-table create --resource-group $GROUP_NAME --name ftnt-demo-routesForPeering
az network route-table route create --name internet --resource-group $GROUP_NAME \
     --route-table-name ftnt-demo-routesForPeering --next-hop-type VirtualAppliance \
...
az network vnet subnet update --vnet-name ftnt-demo-aks  --resource-group $GROUP_NAME  \
   --route-table ftnt-demo-routesForPeering --name ftnt-demo-aks-sub

az network vnet peering create -g  $GROUP_NAME  -n TransitToAKS --vnet-name ftnt-demo-Vnet \
    --remote-vnet ftnt-demo-aks --allow-vnet-access --allow-forwarded-traffic
az network vnet peering create -g  $GROUP_NAME  -n AKStoTransit --vnet-name ftnt-demo-aks \
    --remote-vnet  ftnt-demo-Vnet --allow-vnet-access --allow-forwarded-traffic
...
VNET_ID=$(az network vnet show --resource-group $GROUP_NAME --name  ftnt-demo-aks --query id -o tsv)

# Assign the service principal Contributor permissions to the virtual network resource
az role assignment create --assignee $SP_ID --scope $VNET_ID --role Contributor

AKSSUBNET=`az network vnet subnet list     --resource-group  $GROUP_NAME     --vnet-name ftnt-demo-aks     --query "[0].id" --output tsv`
